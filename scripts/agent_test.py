...
class AgentNode():
    ...
    def path_finding(self):
        ...
        for i in range(self.max_depth):
            if self.path_list[i] == 0:
                ...
                min_temp_cost_list = []
                for j in range(self.num_tasks):
                    if self.temp_cost_list[j] != -1:
                        min_temp_cost_list.append(self.temp_cost_list[j])
                    min_temp_cost_list_sorted = sorted(min_temp_cost_list)            
                    if self.check_list(min_temp_cost_list_sorted, 0):
                        min_temp_cost_list_sorted[0] = max(self.temp_cost_list) + 1
                    if len(min_temp_cost_list_sorted) != 0:    
                        min_cost = min(min_temp_cost_list_sorted)                
                if len(min_temp_cost_list) == 0:
                    break
                ...
                blocked_tasks.add(min_cost_index + 1) 
    ...
    def settings(self):
        ...
        # Final cost lists (list of Costs for each task) <<< MOST IMPORTANT
        self.final_cost_list = [0 for _ in range(self.max_depth)] 
        ...
        # Temp cost list
        self.temp_cost_list = [0 for _ in range(self.num_tasks)]
